...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  const data: { nome?: string; ativo?: boolean; ordem?: number; moeda?: string } = {};
  ...
  if (["USD", "EUR", "BRL"].includes(body.moeda)) data.moeda = body.moeda;
  const r = await prisma.site.updateMany({
...
export async function DELETE(_req: Request, ctx: Ctx) {
  ...
  if (uid === null) return NextResponse.json({ error: "Não autenticado" }, { status: 401 });
  const { id } = await ctx.params;
  await prisma.site.deleteMany({ where: { id: Number(id), usuarioId: uid } });
  return NextResponse.json({ ok: true });
}
